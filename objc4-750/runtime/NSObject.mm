...
// HaveOld= true 对于已经赋值的变量需要先清空
// haveNew = true 有一个新值需要被赋值给这个变量
// crashIfDeallocating = true 这个过程会被终止如果newObj正在被销毁或者newObj的类不支持弱引用
...
objc_object::sidetable_addExtraRC_nolock(size_t delta_rc)
{
    ...
    // 从SideTables中根据对象地址获取对应的SideTable
    SideTable& table = SideTables()[this];
    // 获取当前的引用计数
    size_t& refcntStorage = table.refcnts[this];
    size_t oldRefcnt = refcntStorage;
    ...
    SIDE_TABLE_RC_SHIFT == 2   1ULL<<20 因为 refcnts 中的 64 为的最低两位是有意义的标志位，
     所以在使用 addc 时要将 delta_rc 左移两位，获得一个新的引用计数 newRefcnt。
