...
#endif
		// 字符串比较 即使缓存已经被加载到了共享缓存区域 那么也不匹配
...
{	
	...
	if ( csops(0, CS_OPS_STATUS, &flags, sizeof(flags)) != -1 ) {
		...
		if ( ((flags & CS_RESTRICT) == CS_RESTRICT) && (csr_check(CSR_ALLOW_TASK_FOR_PID) != 0) ) {
			//在macOS系统上，在需要验证代码签名（Gatekeeper开启）的情况下，且csr_check(CSR_ALLOW_TASK_FOR_PID)返回为真（表示Rootless开启了TASK_FOR_PID标志）时，进程才不会受限，在macOS版本10.12系统上，默认Gatekeeper是开启的，并且Rootless是关闭了CSR_ALLOW_TASK_FOR_PID标志位的，这意味着，默认情况下，系统上运行的进程是受限的。
